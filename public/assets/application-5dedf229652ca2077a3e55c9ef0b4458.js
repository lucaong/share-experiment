...
(function() {
  ...
  Cookie = (function() {
    ...
    function Cookie(key, _config) {
      ...
      if ((_ref = (_base = this._config).defaults) == null) {
        _base.defaults = {};
      }
    }
    ...
    Cookie.prototype.get = function() {
      var cookie, cookies, i, name, parts, value, _i, _len;
...
(function() {
  ...
  MessageWidget = (function() {
    ...
    MessageWidget.prototype.render = function() {
      var $li;
      return $li = $("<li>").attr({
        "data-created-at": this.message.created_at,
        "data-time-ago": moment(this.message.created_at).fromNow()
      }).html(this.message.body).prepend($("<span class='author'>").html(this.message.author));
    };
...
(function() {
  var MessageFormWidget;

  MessageFormWidget = (function() {

    function MessageFormWidget(selector, context) {
      $(selector).on("submit keyup", function(event) {
        var $input, author, body, slug;
        ...
        if (body.length) {
          return $.post("/" + slug, {
            ...
            return context.pubsub.publish("/channels/" + slug, data);
          }).fail(function() {
            return alert("Failed sending message");
          });
        }
      });
    }
  ...
  this.MessageFormWidget = MessageFormWidget;

}).call(this);
(function() {
  var AuthorFormWidget;

  AuthorFormWidget = (function() {

    function AuthorFormWidget(selector, context) {
      $(selector).on("submit keyup", function(event) {
        var $form, name;
        if (event.type === "keyup" && event.which !== 13) {
          return true;
    ...
    return AuthorFormWidget;

  })();

  this.AuthorFormWidget = AuthorFormWidget;
...
(function() {
  ...
  ChannelsController = (function(_super) {
    var loopUpdateTimeAgo, scrollToBottom, updateTimeAgo;

    __extends(ChannelsController, _super);

    function ChannelsController() {
      return ChannelsController.__super__.constructor.apply(this, arguments);
    }
    ...
    ChannelsController.prototype.show = function() {
      ...
      scrollToBottom();
      loopUpdateTimeAgo();
      this.pubsub = new Faye.Client("/faye");
      this.authorCookie = new Cookie("" + slug + "_author");
      this.pubsub.subscribe("/channels/" + slug, function(message) {
        var message_widget;
        message_widget = new MessageWidget(message);
        $("ul.messages").append(message_widget.render());
        scrollToBottom();
        return updateTimeAgo();
      });
      new MessageFormWidget("#new_message", this);
      return new AuthorFormWidget("#author_prompt", this);
    };
    ...
    updateTimeAgo = function() {
      return $("ul.messages li").each(function() {
        var $li;
        $li = $(this);
        return $li.attr("data-time-ago", moment($li.data("createdAt")).fromNow());
      });
    };

    loopUpdateTimeAgo = function() {
      updateTimeAgo();
      return setTimeout(loopUpdateTimeAgo, 30000);
    };
...
(function() {
  ...
  Router = new Cartograph(function() {
    this.map("/test", function() {});
    return this.map("/:slug", ChannelsController.action("show"));
  });
